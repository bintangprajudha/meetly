...
const props = defineProps<{
    post: PostData;
    targetPostId: number | string; // post id to repost (handles reposting a repost)
}>();
            ...
            <div class="p-4">
                ...
                <div class="border-l-4 border-gray-300 bg-gray-50 p-3 mb-4 rounded">
                    <div class="flex items-start space-x-3 mb-3">
                        <div class="flex h-8 w-8 items-center justify-center rounded-full bg-blue-500 text-xs font-medium text-white">
                            {{ post.user.name.charAt(0).toUpperCase() }}
                        </div>
                        <div class="flex-1">
                            <p class="font-medium text-sm text-gray-900">{{ post.user.name }}</p>
                            <p class="text-xs text-gray-500">@{{ post.user.email.split('@')[0] }}</p>
                        </div>
                    </div>
                    <p class="text-sm text-gray-800 mb-3">{{ post.content }}</p>
                    <div v-if="post.images?.length" class="grid gap-2" :class="{ 'grid-cols-2': post.images.length > 1 }">
                        <img 
                ...
                <form @submit.prevent="submitRepost" class="space-y-4">
                    ...
                    <div>
                        ...
                        <!-- Image Previews Grid -->
                        <div v-if="imagePreviews.length > 0" class="mt-3 grid gap-2" :class="{ 'grid-cols-1': imagePreviews.length === 1, 'grid-cols-2': imagePreviews.length > 1 }">
                    ...
                    <div class="flex items-center justify-between border-t border-gray-200 pt-4">
                        <label class="cursor-pointer rounded-full p-2 text-blue-500 transition-colors hover:bg-blue-50" :class="{ 'cursor-not-allowed opacity-50': imageFiles.length >= maxImages }">
                            <input
                                id="image-upload"
                                type="file"
                                accept="image/*"
